...
- (BOOL)application:(UIApplication *)application didFinishLaunchingWithOptions:(NSDictionary *)launchOptions {
    ...
    tabBarController.viewControllers = @[controller,controller1];
    self.window.rootViewController = tabBarController;
    [[tabBarController.tabBar.items objectAtIndex:0] setTitle:NSLocalizedString(@"Config",)];
    [[tabBarController.tabBar.items objectAtIndex:1] setTitle:NSLocalizedString(@"Start",)];
...
- (NSPersistentContainer *)persistentContainer {
    ...
    @synchronized (self) {
        if (_persistentContainer == nil) {
            _persistentContainer = [[NSPersistentContainer alloc] initWithName:@"IndoorNavi"];
